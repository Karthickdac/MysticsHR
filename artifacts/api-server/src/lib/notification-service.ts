...
function getDefaultEmailBody(eventType: string, vars: Record<string, string>): string {
  ...
  const bodies: Record<string, string> = {
    ...
    fnf_approved: `${greet}<p>Your Full and Final Settlement has been <strong style="color:green">fully approved</strong>. Your relieving letter and experience certificate have been issued and are available in the MysticsHR documents section.</p>${vars.totalPayable ? `<p>Total payable: <strong>${vars.totalPayable}</strong></p>` : ""}<p>Thank you for your contributions to Automystics Technologies. We wish you the very best in your future endeavours.</p>`,
